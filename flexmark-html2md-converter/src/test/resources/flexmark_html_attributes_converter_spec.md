...
```````````````````````````````` example(Fenced Code: 12) options(code-emphasis)
```html
<html>
<body>
<div>
...
.
<h1>Test<a id="user-content-test"></a></h1>
````````````````````````````````


GitHub Heading ID not Stripped Out

```````````````````````````````` example(ID Prefix: 2) options(no-github-id)
