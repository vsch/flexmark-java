...
public class FlexmarkHtmlConversion {
    ...
    private FlexmarkHtmlConversion(DataHolder options) {
        myOptions = options;
    ...
    private void resetForParse() {
        ...
        Ref<com.vladsch.flexmark.util.ast.Document> document = FOR_DOCUMENT.getFrom(myOptions);
    ...
    public HtmlConverterOptions getOptions() {
        return myHtmlConverterOptions;
    }
    ...
    public void parse(LineFormattingAppendable out, String html) {
        ...
        if (needTrailingEOL && myHtmlConverterOptions.addTrailingEol) {
            out.line();
        }
    }
     ...
     */
    public static FlexmarkHtmlConversion build() {
        return new FlexmarkHtmlConversion(null);
    }
     ...
     * @return html parser instance
     */
    public static FlexmarkHtmlConversion build(DataHolder options) {
        return new FlexmarkHtmlConversion(options);
    }
    ...
    public static String parse(String html, int maxBlankLines, DataHolder options) {
        ...
        FlexmarkHtmlConversion parser = new FlexmarkHtmlConversion(options);
    ...
    private void processElement(LineFormattingAppendable out, Node node) {
        ...
        if (tagParam != null) {
            switch (tagParam.tagType) {
                ...
                case CODE                    : outputHtml = true; if (myHtmlConverterOptions.extInlineCode.isParsed()) processed = processCode(out, (Element) node); break;
                case DEL                     : outputHtml = true; if (myHtmlConverterOptions.extInlineDel.isParsed()) processed = processDel(out, (Element) node); break;
                case DIV                     : processed = processDiv(out, (Element) node); break;
                case DL                      : processed = processDl(out, (Element) node); break;
                case _EMPHASIS               : outputHtml = true; if (myHtmlConverterOptions.extInlineEmphasis.isParsed()) processed = processEmphasis(out, (Element) node); break;
                ...
                case INS                     : outputHtml = true; if (myHtmlConverterOptions.extInlineIns.isParsed()) processed = processIns(out, (Element) node); break;
                case _STRONG_EMPHASIS        : outputHtml = true; if (myHtmlConverterOptions.extInlineStrong.isParsed()) processed = processStrong(out, (Element) node); break;
                case SUB                     : outputHtml = true; if (myHtmlConverterOptions.extInlineSub.isParsed()) processed = processSub(out, (Element) node); break;
                case SUP                     : outputHtml = true; if (myHtmlConverterOptions.extInlineSup.isParsed()) processed = processSup(out, (Element) node); break;
                ...
                case MATH                    : outputHtml = true; if (myHtmlConverterOptions.extMath.isParsed() && myHtmlConverterOptions.extMath != ExtensionConversion.MARKDOWN) processed = processMath(out, (Element) node); break;
        ...
        if (!processed) {
            ...
            } else if (myHtmlConverterOptions.outputUnknownTags) {
    ...
    private boolean processCode(LineFormattingAppendable out, Element element) {
        skip();
        if (!myHtmlConverterOptions.extInlineCode.isSuppressed()) {
            ...
            myInlineCode = true;
            processTextNodes(out, element, false, myHtmlConverterOptions.extInlineCode.isTextOnly() ? "" : backTicks);
    ...
    private boolean processDel(LineFormattingAppendable out, Element element) {
        skip();
        if (!myHtmlConverterOptions.extInlineDel.isSuppressed()) {
            if (!myHtmlConverterOptions.preCodePreserveEmphasis && out.isPreFormatted()) {
                wrapTextNodes(out, element, "", false);
            } else {
                wrapTextNodes(out, element, myHtmlConverterOptions.extInlineDel.isTextOnly() ? "" : "~~", element.nextElementSibling() != null);
            }
        }
        return true;
    }
    ...
    private boolean processEmphasis(LineFormattingAppendable out, Element element) {
        skip();
        if (!myHtmlConverterOptions.extInlineEmphasis.isSuppressed()) {
            if (!myHtmlConverterOptions.preCodePreserveEmphasis && out.isPreFormatted()) {
                wrapTextNodes(out, element, "", false);
            } else {
                wrapTextNodes(out, element, myHtmlConverterOptions.extInlineEmphasis.isTextOnly() ? "" : "*", element.nextElementSibling() != null);
            }
        }
        return true;
    }
    ...
    private boolean processIns(LineFormattingAppendable out, Element element) {
        skip();
        if (!myHtmlConverterOptions.extInlineIns.isSuppressed()) {
            if (!myHtmlConverterOptions.preCodePreserveEmphasis && out.isPreFormatted()) {
                wrapTextNodes(out, element, "", false);
            } else {
                wrapTextNodes(out, element, myHtmlConverterOptions.extInlineIns.isTextOnly() ? "" : "++", element.nextElementSibling() != null);
            }
        }
        return true;
    }
    ...
    private boolean processStrong(LineFormattingAppendable out, Element element) {
        skip();
        if (!myHtmlConverterOptions.extInlineStrong.isSuppressed()) {
            if (!myHtmlConverterOptions.preCodePreserveEmphasis && out.isPreFormatted()) {
                wrapTextNodes(out, element, "", false);
            } else {
                wrapTextNodes(out, element, myHtmlConverterOptions.extInlineStrong.isTextOnly() ? "" : "**", element.nextElementSibling() != null);
            }
        }
        return true;
    }
    ...
    private boolean processSub(LineFormattingAppendable out, Element element) {
        skip();
        if (!myHtmlConverterOptions.extInlineSub.isSuppressed()) {
    ...
    private boolean processSup(LineFormattingAppendable out, Element element) {
        skip();
        if (!myHtmlConverterOptions.extInlineSup.isSuppressed()) {
    ...
    private boolean processMath(LineFormattingAppendable out, Element element) {
        skip();
        if (!myHtmlConverterOptions.extMath.isSuppressed()) {
    ...
    private boolean processSpan(LineFormattingAppendable out, Element element) {
        ...
        processHtmlTree(out, element, true, () -> transferIdToParent());
        return true;
    }
