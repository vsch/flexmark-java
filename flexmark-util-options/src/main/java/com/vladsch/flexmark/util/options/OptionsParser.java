...
public class OptionsParser<T> implements OptionParser<T> {
  ...
  public OptionsParser(
      String optionName,
      OptionParser<T>[] parseableOptions,
  ...
  public Pair<T, List<ParsedOption<T>>> parseOption(
    ...
    for (BasedSequence optionText : optionsList) {
      ...
      BasedSequence optionValue =
          optionList.length > 1
              ? optionList[1]
              : optionName.subSequence(optionName.length(), optionName.length());
        ...
        if (optionParser.getOptionName().startsWith(optionName.toString())) {
          if (matched == null) {
            ...
            if (message == null) {
              ...
              message.append(
                  provider.message(KEY_OPTION_0_IS_AMBIGUOUS, OPTION_0_IS_AMBIGUOUS, optionName));
      ...
      if (matched != null) {
        if (message == null) {
          ...
          parsedOptions.add(
              new ParsedOption<>(
                  optionText, this, ParsedOptionStatus.VALID, null, pair.getSecond()));
        } else {
          parsedOptions.add(
              new ParsedOption<>(
                  optionText,
                  ...
                  new ParserMessage(optionName, ParsedOptionStatus.ERROR, message.toString())));
        }
      } else {
        message = new DelimitedBuilder(", ");
        message.append(
            provider.message(KEY_OPTION_0_DOES_NOT_MATCH, OPTION_0_DOES_NOT_MATCH, optionName));
