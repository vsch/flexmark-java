...
public class MarkdownTable {
  ...
  private final @NotNull ArrayList<TrackedOffset> trackedOffsets = new ArrayList<>();
  ...
  public TableCell getCaptionCell() {
    return caption.rows.size() > 0 && caption.rows.get(0).cells.size() > 0
  ...
  public boolean getHaveCaption() {
    return caption.rows.size() > 0
        && caption.rows.get(0).cells.size() > 0
  ...
  public int getMinColumnsWithoutColumns(boolean withSeparator, int... skipColumns) {
    return aggregateTotalColumnsWithoutColumns(
        withSeparator ? ALL_TABLE_ROWS : ALL_CONTENT_ROWS, MinAggregator.INSTANCE, skipColumns);
  }

  public int getMaxColumnsWithoutColumns(boolean withSeparator, int... skipColumns) {
    return aggregateTotalColumnsWithoutColumns(
        withSeparator ? ALL_TABLE_ROWS : ALL_CONTENT_ROWS, MaxAggregator.INSTANCE, skipColumns);
  }

  public int getMinColumnsWithoutRows(boolean withSeparator, int... skipRows) {
    return aggregateTotalColumnsWithoutRows(
        withSeparator ? ALL_TABLE_ROWS : ALL_CONTENT_ROWS, MinAggregator.INSTANCE, skipRows);
  }

  public int getMaxColumnsWithoutRows(boolean withSeparator, int... skipRows) {
    return aggregateTotalColumnsWithoutRows(
        withSeparator ? ALL_TABLE_ROWS : ALL_CONTENT_ROWS, MaxAggregator.INSTANCE, skipRows);
  }
  ...
  */
  ...
  public MarkdownTable sorted(
    ...
    Comparator<TableRow> rowComparator =
        (o1, o2) -> {
          for (ColumnSort columnSort : columnSorts) {
            int c = columnSort.column;
            if (c >= 0 && c < cMax) {
  ...
  public void appendTable(LineAppendable out) {
    ...
    if (captionText != null) {
      BasedSequence formattedCaption =
          BasedSequence.of(captionText).subSequence(0, ((CharSequence) captionText).length());
  ...
  private void appendRows(
      LineAppendable out, List<TableRow> rows, boolean isHeader, CharSequence linePrefix) {
  ...
  private int aggregateTotalColumnsWithoutRows(
      TableSection[] sections, BiFunction<Integer, Integer, Integer> aggregator, int... skipRows) {
    Integer[] columns = new Integer[] {null};
