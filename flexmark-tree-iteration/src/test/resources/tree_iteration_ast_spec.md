---
title: TreeIteration Extension Spec
author: 
version: 
date: '2016-06-06'
...
Converts tree_iteration text to TreeIteration nodes.

no spaces between brackets

```````````````````````````````` example(TreeIteration: 1) options(option1, IGNORE)
Sample  text
.
...
```````````````````````````````` example(TreeIteration: 2) options(FAIL)
&#X01; &#X234564; fail
.
<p> �</p>
.
