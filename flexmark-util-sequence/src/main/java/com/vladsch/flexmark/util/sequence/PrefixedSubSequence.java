 ...
 *
 * <p>a subSequence() returns a sub-sequence from the original base sequence, possibly with a prefix
...
public final class PrefixedSubSequence extends BasedSequenceImpl implements ReplacedBasedSequence {
  ...
  public int getIndexOffset(int index) {
    ...
    if (index < prefix.length()) {
      // NOTE: to allow creation of segmented sequences from modified original base return -1 for
      // all such modified content positions
      return -1;
    }
  ...
  public static PrefixedSubSequence repeatOf(
      CharSequence prefix, int count, BasedSequence baseSeq) {
    return prefixOf(
        RepeatedSequence.repeatOf(prefix, count).toString(), baseSeq, 0, baseSeq.length());
  }
