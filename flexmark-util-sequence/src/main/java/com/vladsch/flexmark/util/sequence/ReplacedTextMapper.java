 ...
 *
 * <p>This is needed when the original based sequence needs to be un-escaped but offsets to original
 * escaped text are needed.
 *
 * <p>These replacements can be nested so that you can track replacements of replaced text. To add
 * nested replacements use startNestedReplacement()
 *
 * <p>when isModified() returns true then the text mapper is already used and nested replacements
 * need to be applied
 */
// REFACTOR: to use segment builder with ISegmentBuilder.F_INCLUDE_ANCHORS and use segment
// information to find original offsets
public class ReplacedTextMapper {
  ...
  private void finalizeMods() {
    if (replacedSequence == null) {
      replacedSequence =
          replacedSegments.isEmpty()
              ? BasedSequence.NULL
              : SegmentedSequence.create(original, replacedSegments);
    }
  }
  ...
  public void addReplacedText(int startIndex, int endIndex, BasedSequence replacedSequence) {
    ...
    regions.add(
        new ReplacedTextRegion(
            ...
            Range.of(replacedLength, replacedLength + replacedSequence.length())));
    ...
    if (startIndex < endIndex) {
      ...
      regions.add(
          new ReplacedTextRegion(
              ...
              Range.of(startIndex, endIndex),
              Range.of(replacedLength, replacedLength + originalSegment.length())));
  ...
  public int originalOffset(int replacedIndex) {
    ...
    for (ReplacedTextRegion region : regions) {
      if (region.containsReplacedIndex(replacedIndex)) {
        originalIndex =
            region.getOriginalRange().getStart()
                + replacedIndex
                - region.getReplacedRange().getStart();
