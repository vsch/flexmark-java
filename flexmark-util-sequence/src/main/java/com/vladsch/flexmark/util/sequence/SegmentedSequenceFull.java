...
public final class SegmentedSequenceFull extends SegmentedSequence {
  ...
  private final int
      baseStartOffset; // start offset for baseOffsets of this sequence, offset from baseSeq for all

  // chars, including non-base chars
  ...
  private static SegmentedSequenceFull subSequence(
    ...
    if (!nonBaseChars) {
      ...
      if (!finished) {
        // if no real start after then it is the base's end since we had no real start after, these
        // chars and after are all out of base chars
        startOffset = baseSeq.getEndOffset();
        endOffset = startOffset;
      }
    }
