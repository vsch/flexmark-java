...
public interface NodeRendererContext extends LinkResolverContext {
   ...
   * @param attributes the attributes that were calculated by the renderer or null, these may be
   ...
   * context inherits everything but the HtmlRenderer, renderedNode, renderingHandler and
   * doNotRenderLinksNesting from the parent.
   *
   * @param inheritIndent whether the html writer of the sub-context should inherit the current
  ...
  /**
   * Creates a child rendering context that can be used to collect rendered html text of the
   * previously registered node renderer. The child context inherits everything but the HtmlRenderer
   * and doNotRenderLinksNesting from the parent.
   *
   * @param inheritIndent whether the html writer of the sub-context should inherit the current
   *     context's indentation level or start with 0 indentation
   * @return a new rendering context with a given appendable for its output
   ...
   *
   * @param node node for which to get an id, depends on the {@link HtmlIdGenerator} assigned for
   *     the context. Default generator only creates ids for {@link
   *     com.vladsch.flexmark.ast.Heading} nodes or custom nodes that implement {@link
   ...
   * would render a link, should only output the text of that link and image links should not render
   * anything.
   *
  ...
  /**
   * Increment/Decrement the do not render links in this context. This value will persist for the
   * duration of the current node's render() method and will be restored upon return. Effectively it
   * will persist for the rendering of the children of this node.
   *
  ...
  /**
   * Increment the do not render links in this context. This value will persist for the duration of
   * the current node's render() method and will be restored upon return. Effectively it will
   * persist for the rendering of the children of this node.
   *
   * @see #isDoNotRenderLinks()
   * @see #doNotRenderLinks()
   * @see #doRenderLinks()
  ...
  /**
   * Decrement the do not render links in this context. This value will persist for the duration of
   * the current node's render() method and will be restored upon return. Effectively it will
   * persist for the rendering of the children of this node.
   *
