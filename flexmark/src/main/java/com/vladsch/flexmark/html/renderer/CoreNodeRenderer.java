...
public class CoreNodeRenderer implements NodeRenderer {
  ...
  private static final AttributablePart PARAGRAPH_LINE = new AttributablePart("PARAGRAPH_LINE");
  ...
  private CoreNodeRenderer(DataHolder options) {
  ...
  private void render(FencedCodeBlock node, NodeRendererContext context, HtmlWriter html) {
    ...
    if (info.isNotNull() && !info.isBlank()) {
      ...
      String languageClass =
          htmlOptions.languageClassMap.getOrDefault(
              language, htmlOptions.languageClassPrefix + language);
  ...
  private void render(OrderedList node, NodeRendererContext context, HtmlWriter html) {
    ...
    if (listOptions.isOrderedListManualStart() && start != 1)
  ...
  private void renderTextBlockParagraphLines(
  ...
  private void outputSourceLineSpan(
      Node parentNode, Node startNode, Node endNode, HtmlWriter html) {
    ...
    if (range.getEnd() <= endOffset) {
      ...
      endOffset -= eolLength;
      endOffset -=
          parentNode.baseSubSequence(startOffset, endOffset).countTrailing(CharPredicate.SPACE_TAB);
      myNextLine++;
      nextLineStartOffset = range.getEnd();
      nextLineStartOffset +=
          parentNode
              .baseSubSequence(nextLineStartOffset, parentNode.getEndOffset())
              .countLeading(CharPredicate.SPACE_TAB);
    }
  ...
  private void outputNextLineBreakSpan(Node node, HtmlWriter html, boolean outputBreakText) {
    ...
    // remove trailing spaces from text
    int countTrailing =
        node.baseSubSequence(nextLineStartOffset, range.getEnd() - eolLength)
            .countTrailing(CharPredicate.SPACE_TAB);
    if (!outputBreakText && countTrailing > 0) {
      countTrailing--;
    }
    eolLength += countTrailing;

    html.srcPos(nextLineStartOffset, range.getEnd() - eolLength)
        .withAttr(PARAGRAPH_LINE)
        .tag("span");
    nextLineStartOffset = range.getEnd();

    // remove leading spaces
    nextLineStartOffset +=
        node.baseSubSequence(nextLineStartOffset, node.getChars().getBaseSequence().length())
            .countLeading(CharPredicate.SPACE_TAB);
  }
  ...
  private void renderLooseParagraph(Paragraph node, NodeRendererContext context, HtmlWriter html) {
    if (context.getHtmlOptions().noPTagsUseBr) {
      ...
      html.srcPosWithEOL(node.getChars())
          .withAttr()
          .tagLine("p", () -> renderTextBlockParagraphLines(node, context, html, false));
    }
  }
  ...
  private void render(Paragraph node, NodeRendererContext context, HtmlWriter html) {
    if (node.getFirstChildAnyNot(NonRenderingInline.class) != null) {
      if (!(node.getParent() instanceof ParagraphItemContainer)
          || !((ParagraphItemContainer) node.getParent())
  ...
  private boolean renderLineBreak(
      String breakText,
      String suppressInTag,
      Node node,
    ...
    if (myLines != null && myNextLine < myLines.size()) {
      ...
      boolean outputBreakText =
          iMax == 0
              || suppressInTag == null
              || !suppressInTag.equalsIgnoreCase(openTags.get(iMax - 1));
      ...
      for (String tag : openTags) {
        if (!outputBreakText
            && context.getHtmlOptions().inlineCodeSpliceClass != null
            && !context.getHtmlOptions().inlineCodeSpliceClass.isEmpty()) {
          html.attr(Attribute.CLASS_ATTR, context.getHtmlOptions().inlineCodeSpliceClass)
              .withAttr()
              .tag(tag);
        } else {
          html.tag(tag);
  ...
  private void render(StrongEmphasis node, NodeRendererContext context, HtmlWriter html) {
    HtmlRendererOptions htmlOptions = context.getHtmlOptions();
    if (htmlOptions.strongEmphasisStyleHtmlOpen == null
        || htmlOptions.strongEmphasisStyleHtmlClose == null) {
      if (context.getHtmlOptions().sourcePositionParagraphLines) {
  ...
  private void render(HtmlBlock node, NodeRendererContext context, HtmlWriter html) {
    ...
    if (htmlOptions.sourceWrapHtmlBlocks) {
      html.srcPos(node.getChars())
          .withAttr(AttributablePart.NODE_POSITION)
          .tag("div")
          .indent()
          .line();
    }
    ...
    if (node.hasChildren()) {
      ...
      renderHtmlBlock(
          node, context, html, htmlOptions.suppressHtmlBlocks, htmlOptions.escapeHtmlBlocks, false);
    }
  ...
  private void render(HtmlInnerBlock node, NodeRendererContext context, HtmlWriter html) {
    renderHtmlBlock(
        ...
        context,
        html,
        context.getHtmlOptions().suppressHtmlBlocks,
        context.getHtmlOptions().escapeHtmlBlocks,
        false);
  }
  ...
  private static void renderHtmlBlock(
      HtmlBlockBase node,
      NodeRendererContext context,
      HtmlWriter html,
      boolean suppress,
    ...
    String normalizeEOL =
        node instanceof HtmlBlock
            ? node.getContentChars().normalizeEOL()
            : node.getChars().normalizeEOL();
  ...
  private static boolean isSuppressedLinkPrefix(CharSequence url, NodeRendererContext context) {
  ...
  private void render(AutoLink node, NodeRendererContext context, HtmlWriter html) {
    ...
    if (context.isDoNotRenderLinks() || isSuppressedLinkPrefix(node.getUrl(), context)) {
      ...
      html.srcPos(node.getText())
          .attr(
              "href",
              resolvedLink.getUrl().startsWith("www.")
                  ? context.getHtmlOptions().autolinkWwwPrefix + resolvedLink.getUrl()
                  : resolvedLink.getUrl())
  ...
  private void render(Image node, NodeRendererContext context, HtmlWriter html) {
    if (!(context.isDoNotRenderLinks() || isSuppressedLinkPrefix(node.getUrl(), context))) {
      ...
      ResolvedLink resolvedLink =
          context.resolveLink(LinkType.IMAGE, node.getUrl().unescape(), null, null);
      String url = resolvedLink.getUrl();

      if (!node.getUrlContent().isEmpty()) {
        // reverse URL encoding of =, &
        String content =
            Escaping.percentEncodeUrl(node.getUrlContent())
                .replace("+", "%2B")
                .replace("%3D", "=")
                .replace("%26", "&amp;");
        url += content;
  ...
  private void renderChildrenSourceLineWrapped(
    ...
    if (context.getHtmlOptions().sourcePositionParagraphLines
        && nodeChildText.indexOfAny(CharPredicate.ANY_EOL) >= 0) {
      if (myNextLine > 0) {
        myNextLine--;
      }
  ...
  private void render(ImageRef node, NodeRendererContext context, HtmlWriter html) {
    ...
    if (resolvedLink == null) {
      ...
      if (!(context.isDoNotRenderLinks() || isSuppressed)) {
        ...
        if (reference != null) {
          attributes =
              context.extendRenderingNodeAttributes(reference, AttributablePart.NODE, attributes);
        }
