...
public interface LinkResolverContext extends LinkResolverBasicContext {
   ...
   * the {@link HtmlRenderer#builder()} or passed to {@link HtmlRenderer#builder(DataHolder)}. To
   * get the document options you should use {@link #getDocument()} as the data holder.
   *
  ...
  /**
   * Resolve link for rendering. Link Resolvers are going to be called until one returns
   * ResolvedLink with getStatus() != LinkStatus.Unknown
   *
   * <p>A resolver can replace the url but not change the status letting downstream resolvers handle
   * the rest. This is useful when a resolver does partial processing like macro expansion but does
   * not know how to handle the rest.
   *
   * <p>Core processing will simply pass the link as is. It is up to extension LinkResolvers and
   * AttributeProviders to make sense of the link and applicable attributes based on status.
   *
   * @param linkType type of link being rendered. Core defined links are Link, Image. Extensions can
   *     define their own
   * @param url link url text
   * @param urlEncode whether the link should be url encoded, if null then the value of {@link
   *     HtmlRenderer#PERCENT_ENCODE_URLS} will be used to determine whether the resolved URL is to
   *     be encoded.
