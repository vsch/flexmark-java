...
public class CoreNodeFormatter
    ...
    return new HashSet<>(
        Arrays.asList(
            ...
            new NodeFormattingHandler<>(
                HtmlInnerBlockComment.class, CoreNodeFormatter.this::render),
  ...
  @Override
  public void renderReferenceBlock(
    ...
    if (context.isTransformingText()) {
      ...
      if (context.getRenderPurpose() == RenderPurpose.TRANSLATION_SPANS) {
        ...
        if (resolvedLink.getAnchorRef() != null) {
          ...
          CharSequence anchorRef =
              context.transformAnchorRef(resolvedLink.getPageRef(), resolvedLink.getAnchorRef());
          if (attributeUniquificationIdMap != null
              && resolvedLink.getPageRef().isEmpty()
              && context.isTransformingText()
              && context.getMergeContext() != null) {
            String stringAnchorRef = String.valueOf(anchorRef);
            String uniquifiedAnchorRef =
                attributeUniquificationIdMap.getOrDefault(stringAnchorRef, stringAnchorRef);
      ...
      if (next instanceof HtmlCommentBlock || next instanceof HtmlInnerBlockComment) {
        ...
        if (formatterOptions.linkMarkerCommentPattern != null
            && formatterOptions.linkMarkerCommentPattern.matcher(text).matches()) {
  ...
  public void renderDocument(
    ...
    attributeUniquificationIdMap =
        Formatter.ATTRIBUTE_UNIQUIFICATION_ID_MAP.get(context.getTranslationStore());
          ...
          if (key.get(document) instanceof NodeRepository) {
            ...
            for (Object value : nodes) {
              if (value instanceof Node) {
                ...
                // NOTE: here the node.getDocument() is necessary to test if this was appended
                // reference
  ...
  private void render(BlankLine node, NodeFormatterContext context, MarkdownWriter markdown) {
    if (FormatterUtils.LIST_ITEM_SPACING.get(context.getDocument()) == null
        && markdown.offsetWithPending() > 0) {
  ...
  private void render(Heading node, NodeFormatterContext context, MarkdownWriter markdown) {
    ...
    if (context.isTransformingText()
        || headingPreference.isNoChange(node.isSetextHeading(), node.getLevel())) {
      if (node.isAtxHeading()) {
        markdown.append(node.getOpeningMarker());
        boolean spaceAfterAtx =
            formatterOptions.spaceAfterAtxMarker == ADD
                || formatterOptions.spaceAfterAtxMarker == AS_IS
                    && node.getOpeningMarker().getEndOffset() < node.getText().getStartOffset();

        if (spaceAfterAtx) markdown.append(' ');

        context.translatingRefTargetSpan(
            node, (context12, writer) -> context12.renderChildren(node));
        ...
        if (formatterOptions.setextHeadingEqualizeMarker) {
          markdown.append(
              node.getClosingMarker().charAt(0),
              Utils.minLimit(
                  markdown.getLineInfo(markdown.getLineCountWithPending() - 1).textLength,
                  formatterOptions.minSetextMarkerLength));
        } else {
          markdown.append(node.getClosingMarker());
        }
      }
      ...
      if (formatterOptions.setextHeadingEqualizeMarker) {
        ...
        markdown.append(
            RepeatedSequence.repeatOf(closingMarker, formatterOptions.minSetextMarkerLength));
      }
    } else {
      // change to atx
      ...
      boolean spaceAfterAtx =
          formatterOptions.spaceAfterAtxMarker == ADD
              || formatterOptions.spaceAfterAtxMarker == AS_IS
                  && !Parser.HEADING_NO_ATX_SPACE.get(context.getOptions());
  ...
  private void render(FencedCodeBlock node, NodeFormatterContext context, MarkdownWriter markdown) {
    ...
    char closingMarkerChar =
        closingMarker.length() > 0 ? closingMarker.charAt(0) : SequenceUtils.NUL;
    ...
    if (openingMarkerLen < formatterOptions.fencedCodeMarkerLength)
      openingMarkerLen = formatterOptions.fencedCodeMarkerLength;
    if (closingMarkerLen < formatterOptions.fencedCodeMarkerLength)
      closingMarkerLen = formatterOptions.fencedCodeMarkerLength;
    ...
    if (formatterOptions.fencedCodeMatchClosingMarker || closingMarkerChar == SequenceUtils.NUL) {
      ...
      closingMarker =
          RepeatedSequence.repeatOf(String.valueOf(closingMarkerChar), closingMarkerLen);
    ...
    if (!(node.getParent() instanceof ListItem)
        || !isLastOfItem(node)
        || LIST_ITEM_SPACING.get(context.getDocument()) == ListSpacing.LOOSE) {
      markdown.tailBlankLine();
    }
  }

  private void render(
      IndentedCodeBlock node, NodeFormatterContext context, MarkdownWriter markdown) {
    markdown.blankLine();

    if (context.isTransformingText()) {
      // here we need actual prefix in the code or the generated partial doc will not be accurate
      // and cause translated AST to be wrong
      ...
      if (formatterOptions.emulationProfile == ParserEmulationProfile.GITHUB_DOC) {
        if (node.getParent() instanceof ListItem) {
          ...
          prefix =
              RepeatedSequence.repeatOf(" ", Utils.minLimit(8 - marker.length() - 1, 4)).toString();
        }
      }
    ...
    if (context.isTransformingText()) {
      ...
      if (!(node.getParent() instanceof ParagraphItemContainer)) {
        if (node.getParent() instanceof ParagraphContainer) {
          boolean startWrappingDisabled =
              ((ParagraphContainer) node.getParent()).isParagraphStartWrappingDisabled(node);
          boolean endWrappingDisabled =
              ((ParagraphContainer) node.getParent()).isParagraphEndWrappingDisabled(node);
          ...
          if (!node.isTrailingBlankLine()
              && (node.getNext() == null || node.getNext() instanceof ListBlock)) {
            FormatterUtils.renderTextBlockParagraphLines(node, context, markdown);
          } else {
            FormatterUtils.renderLooseParagraph(node, context, markdown);
          }
        ...
        if (isItemParagraph) {
          if (formatterOptions.blankLinesInAst) {
            ...
            if (itemSpacing == ListSpacing.TIGHT) {
              ...
              if (!((ParagraphItemContainer) node.getParent())
                  .isParagraphWrappingDisabled(node, listOptions, context.getOptions())) {
                FormatterUtils.renderLooseItemParagraph(node, context, markdown);
              } else {
                FormatterUtils.renderTextBlockParagraphLines(node, context, markdown);
              }
  ...
  private void render(HtmlEntity node, NodeFormatterContext context, MarkdownWriter markdown) {
    if (context.getRenderPurpose() == RenderPurpose.FORMAT) {
      ...
      context.customPlaceholderFormat(
          ...
          (context1, markdown1) -> markdown1.appendNonTranslating(node.getChars()));
    }
  }
  ...
  private void render(
      HtmlCommentBlock node, NodeFormatterContext context, MarkdownWriter markdown) {
    ...
    if (!context.isTransformingText()
        && formatterOptions.linkMarkerCommentPattern != null
        && formatterOptions.linkMarkerCommentPattern.matcher(text).matches()) {
      // if after ref then output nothing, the ref takes care of this
      if (!(node.getPrevious() instanceof Reference)) {
        markdown
            .append("<!--")
            .append(String.valueOf(text.toMapped(SpaceMapper.toNonBreakSpace)))
            .append("-->");
      }
    } else {
  ...
  private void render(HtmlBlockBase node, NodeFormatterContext context, MarkdownWriter markdown) {
    switch (context.getRenderPurpose()) {
      ...
      case TRANSLATED_SPANS:
        markdown.appendNonTranslating(
            myHtmlBlockPrefix, node.getChars().trimEOL(), ">", node.getChars().trimmedEOL());
        break;
      ...
      case FORMAT:
      default:
        // NOTE: to allow changing node chars before formatting, need to make sure the node's chars
        // were not changed before using content lines
        markdown.openPreFormatted(true);
  ...
  private void render(
      HtmlInnerBlockComment node, NodeFormatterContext context, MarkdownWriter markdown) {
    // here we need to make it translating, it is a comment
    BasedSequence text = node.getChars().trim().midSequence(4, -3);
    if (!context.isTransformingText()
        && formatterOptions.linkMarkerCommentPattern != null
  ...
  private void render(
      HtmlInlineComment node, NodeFormatterContext context, MarkdownWriter markdown) {
  ...
  private void renderAutoLink(
      DelimitedLinkNode node,
      ...
      switch (context.getRenderPurpose()) {
        case TRANSLATION_SPANS:
          if (node.getOpeningMarker().isNull()) {
            // unwrapped, need to store that fact, placeholder is used to allow same url
            // wrapped/unwrapped preservation
        ...
        case TRANSLATED:
          // NOTE: on entry the node text is a placeholder, so we can check if it is wrapped or
          // unwrapped
          if (UNWRAPPED_AUTO_LINKS.get(myTranslationStore)
              && UNWRAPPED_AUTO_LINKS_MAP
                  .get(myTranslationStore)
                  .contains(node.getText().toString())) {
            markdown.appendNonTranslating(prefix, node.getText(), suffix);
          } else {
            markdown.append("<");
            markdown.appendNonTranslating(prefix, node.getText(), suffix);
  ...
  private void render(Image node, NodeFormatterContext context, MarkdownWriter markdown) {
    if (!context.isTransformingText()
        && formatterOptions.rightMargin > 0
        && formatterOptions.keepImageLinksAtStart) {
  ...
  private void render(Link node, NodeFormatterContext context, MarkdownWriter markdown) {
    if (!context.isTransformingText()
        && formatterOptions.rightMargin > 0
        && formatterOptions.keepExplicitLinksAtStart) {
      markdown.append(SequenceUtils.LS);
    } else {
      ...
      if (context.getRenderPurpose() == RenderPurpose.TRANSLATION_SPANS) {
        ...
        if (node.getAnchorRef().isNotNull()) {
          ...
          if (attributeUniquificationIdMap != null
            ...
            if (pageRef.length() == 0) {
              uniquifiedAnchorRef =
                  attributeUniquificationIdMap.getOrDefault(
                      uniquifiedAnchorRef, uniquifiedAnchorRef);
            }
            markdown.append(uniquifiedAnchorRef);
          } else {
            markdown.append(anchorRef);
      ...
      if (context.isTransformingText() || formatterOptions.rightMargin == 0) {
        if (node.isReferenceTextCombined()) {
          ...
          FormatterUtils.appendWhiteSpaceBetween(
              markdown, node.getReferenceOpeningMarker(), node.getReference(), true, false, false);
          appendReference(node.getReference(), context, markdown);
          markdown.append(node.getReferenceClosingMarker());
