...
public class FormatterUtils {
  ...
  public static String getBlockLikePrefix(
      ...
      case AS_IS:
        if (node.getFirstChild() != null) {
          usePrefix =
              node.getChars()
                  .baseSubSequence(
                      node.getOpeningMarker().getStartOffset(),
                      node.getFirstChild().getStartOffset())
                  .toString();
        } else {
    ...
    if (compactPrefix
        && combinedPrefix.endsWith(" ")
        && combinedPrefix.length() >= 2
        && quoteLikePrefixPredicate.test(combinedPrefix.charAt(combinedPrefix.length() - 2))) {
  ...
  public static void renderList(
    ...
    int startingNumber =
        node instanceof OrderedList
            ? formatterOptions.listRenumberItems && formatterOptions.listResetFirstItemNumber
                ? 1
                : ((OrderedList) node).getStartNumber()
            : 1;
    Function<CharSequence, Pair<Integer, Integer>> listAlignNumeric =
        LIST_ALIGN_NUMERIC.get(document);
    ...
    if (!formatterOptions.listAlignNumeric.isNoChange() && node instanceof OrderedList) {
      ...
      for (Node item : itemList) {
        if (!formatterOptions.listRemoveEmptyItems
            || item.hasChildren() && item.getFirstChildAnyNot(BlankLine.class) != null) {
          int length =
              formatterOptions.listRenumberItems
                  ? Integer.toString(i).length() + 1
                  : ((ListItem) item).getOpeningMarker().length();
    ...
    document.set(
        LIST_ITEM_SPACING,
        itemSpacing == LOOSE && (listSpacing == null || listSpacing == LOOSE)
            ? LOOSE
            : itemSpacing);
    for (Node item : itemList) {
  ...
  public static void renderLooseParagraph(
  ...
  public static void renderLooseItemParagraph(
      Paragraph node, NodeFormatterContext context, MarkdownWriter markdown) {
    ...
    if (parent instanceof ListItem) {
      if (context.getFormatterOptions().blankLinesInAst) {
        ...
        boolean canAddTailBlankLine =
            !((ParagraphContainer) parent).isParagraphEndWrappingDisabled(node);
        ...
        switch (context.getFormatterOptions().listSpacing) {
          case LOOSEN:
            addBlankLine =
                parent.getParent() instanceof ListBlock
                    && ((ListBlock) parent.getParent()).isLoose()
                    && hasLooseItems(parent.getParent().getChildren())
                    && (isFollowedByBlankLine(node) && isNotLastItem(parent)
                        || !listItem.isOwnTight()
                        || listItem.isItemParagraph(node)
          ...
          case TIGHTEN:
            addBlankLine =
                canAddTailBlankLine
                    && (listItem.isItemParagraph(node)
                        ? isFollowedByBlankLine(node) && isNotLastItem(node)
                        : isNotLastItem(node));
            break;
  ...
  public static void renderListItem(
    ...
    if (context.isTransformingText()) {
      ...
      if (node.getFirstChild() == null) {
        ...
        int count =
            openingMarker.length()
                + (listOptions.isItemContentAfterSuffix() ? markerSuffix.length() : 0)
                + 1;
        ...
        itemContentPrefix =
            getAdditionalPrefix(
                markerSuffix.isEmpty() ? openingMarker : markerSuffix, childContent);
        prefix = additionalPrefix + itemContentPrefix;

        itemContentSpacer =
            getAdditionalPrefix(
                markerSuffix.isEmpty()
                    ? openingMarker.getEmptySuffix()
                    : markerSuffix.getEmptySuffix(),
                childContent);
      }
      ...
      if (!markerSuffix.isEmpty()) {
        String markerSuffixIndent =
            getAdditionalPrefix(openingMarker.getEmptySuffix(), markerSuffix);
      ...
      if (options.listRemoveEmptyItems
      ...
      if (node.isOrderedItem()) {
        ...
        if (padding.getFirst() > 0)
          useOpeningMarker =
              RepeatedSequence.ofSpaces(padding.getFirst()).toString()
                  + useOpeningMarker.toString();
        if (padding.getSecond() > 0)
          useOpeningMarker =
              useOpeningMarker.toString()
                  + RepeatedSequence.ofSpaces(padding.getSecond()).toString();
      } else {
      ...
      // NOTE: if list item content after suffix is set in the parser, then sub-items are indented
      // after suffix
      //    otherwise only the item's lazy continuation for the paragraph can be indented after
      // suffix, child items are normally indented
      int itemContinuationCount =
          (listOptions.isItemContentAfterSuffix() || options.listsItemContentAfterSuffix
              ? markerSuffix.length()
              : 0);
      int continuationCount =
          useOpeningMarker.length()
              + (listOptions.isItemContentAfterSuffix() ? markerSuffix.length() : 0)
              + 1;
      CharSequence additionalItemPrefix =
          options.itemContentIndent ? RepeatedSequence.repeatOf(' ', itemContinuationCount) : "";

      CharSequence childPrefix =
          options.itemContentIndent
              ? RepeatedSequence.repeatOf(' ', continuationCount)
              : RepeatedSequence.repeatOf(" ", listOptions.getItemIndent()).toString();

      BasedSequence openingMarker = node.getOpeningMarker();
      BasedSequence replacedOpenMarker =
          openingMarker
              .getBuilder()
              .append(openingMarker.getEmptyPrefix())
              .append(useOpeningMarker)
              .append(openingMarker.getEmptySuffix())
              .toSequence();
      ...
      if (childNode != null && node.getFirstChildAnyNot(BlankLine.class) != null) {
        ...
        if (addBlankLineLooseItems
            && (node.isLoose() && context.getFormatterOptions().listSpacing == LOOSEN
                || context.getFormatterOptions().listSpacing == LOOSE)) {
          markdown.tailBlankLine();
        }
      } else {
  ...
  public static void renderTextBlockParagraphLines(
      Node node, NodeFormatterContext context, MarkdownWriter markdown) {
    if (context.isTransformingText()) {
      ...
      if (formatterOptions.rightMargin > 0) {
        MutableDataHolder subContextOptions =
            context
                .getOptions()
                .toMutable()
                .set(Formatter.KEEP_SOFT_LINE_BREAKS, true)
                .set(Formatter.KEEP_HARD_LINE_BREAKS, true);
        SequenceBuilder builder = context.getDocument().getChars().getBuilder();
        NodeFormatterContext subContext =
            context.getSubContext(subContextOptions, builder.getBuilder());
        ...
        if (trailingSpaces.isNotEmpty() && !subContextMarkdown.endsWithEOL()) {
          // add these so our tracked offsets at end of paragraph after whitespaces are not outside
          // the sequence
        ...
        if (haveAltSequence) {
          ...
          int endOffsetDelta =
              nodeLessEol.countTrailingWhitespace() - charsLessEol.countTrailingWhitespace();
        ...
        TrackedOffsetList paragraphTrackedOffsets =
            trackedOffsets.getTrackedOffsets(startOffset, endOffset);

        MarkdownParagraph formatter =
            new MarkdownParagraph(
                paragraphChars, altParagraphChars, formatterOptions.charWidthProvider);
        ...
        if (!paragraphTrackedOffsets.isEmpty()) {
          ...
          for (TrackedOffset trackedOffset : paragraphTrackedOffsets) {
            if (trackedOffset.isResolved()) {
              ...
              int prefixDelta =
                  lineInfo.sumPrefixLength
                      - startLineInfo.sumPrefixLength
  ...
  public static void renderBlockQuoteLike(
      BlockQuoteLike node, NodeFormatterContext context, MarkdownWriter markdown) {
    FormatterOptions formatterOptions = context.getFormatterOptions();

    String combinedPrefix =
        getBlockLikePrefix(node, context, formatterOptions.blockQuoteMarkers, markdown.getPrefix());
    ...
    if (!FIRST_LIST_ITEM_CHILD.get(context.getDocument())) {
      ...
      String firstPrefix =
          getBlockLikePrefix(node, context, formatterOptions.blockQuoteMarkers, NULL);
      markdown
          .pushOptions()
    ...
    if (formatterOptions.blockQuoteBlankLines
        && (lines < markdown.getLineCountWithPending()
            && !FIRST_LIST_ITEM_CHILD.get(context.getDocument()))) markdown.tailBlankLine();
  }
}
