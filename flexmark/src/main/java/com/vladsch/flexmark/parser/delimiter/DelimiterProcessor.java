...
public interface DelimiterProcessor {
  ...
  /**
   * @return Minimum number of delimiter characters that are needed to activate this. Must be at
   *     least 1.
   */
  int getMinLength();
   ...
   *
   * <p>This allows implementations to decide how many characters to use based on the properties of
   * the delimiter runs. An implementation can also return 0 when it doesn't want to allow this
   * particular combination of delimiter runs.
   *
   * @param opener the opening delimiter run
   * @param closer the closing delimiter run
   * @return how many delimiters should be used; must not be greater than length of either opener or
   *     closer
   */
  int getDelimiterUse(DelimiterRun opener, DelimiterRun closer);

  /**
   * Process the matched delimiters, e.g. by wrapping the nodes between opener and closer in a new
   * node, or appending a new node after the opener.
   *
   * <p>Note that removal of the delimiter from the delimiter nodes and unlinking them is done by
   * the caller.
   *
   ...
   * @return node to replace unmatched delimiter, null or delimiter.getNode() to replace with
   *     delimiter text
   */
