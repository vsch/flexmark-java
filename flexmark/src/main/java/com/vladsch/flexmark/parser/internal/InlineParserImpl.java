...
public class InlineParserImpl extends LightInlineParserImpl
  ...
  public InlineParserImpl(
    ...
    this.inlineParserExtensionFactories =
        !inlineParserExtensionFactories.isEmpty() ? inlineParserExtensionFactories : null;

    if (this.options.useHardcodedLinkAddressParser) {
      this.linkDestinationParser =
          new LinkDestinationParser(
              this.options.linksAllowMatchedParentheses,
              this.options.spaceInLinkUrls,
              this.options.parseJekyllMacrosInUrls,
              this.options.intellijDummyIdentifier);
    }
  }
  ...
  public int preProcessBlock(Paragraph block, ParserState state) {
    ...
    while (leadingSpaces <= 3
        && length > 3 + leadingSpaces
        && contentChars.charAt(leadingSpaces) == '[') {
      if (leadingSpaces > 0) {
  ...
  protected int parseReference(Block block, BasedSequence s) {
    ...
    // NOTE: whether first or last reference is kept is defined by the repository modify behavior
    // setting
  ...
  protected boolean parseInline(boolean customOnly) {
    ...
    if (customCharacters != null && customCharacters.get(c)) {
      ...
      if (!res) {
        index++;
        // When we get here, it's only for a single special character that turned out to not have a
        // special meaning.
        // So we shouldn't have a single surrogate here, hence it should be ok to turn it into a
        // String.
        appendText(input.subSequence(index - 1, index));
      } else {
        // Issue: #376, need to clear any delimiter stack since whatever is not used is not a
        // delimiter
        processDelimiters(null);
        this.lastDelimiter = null;
      }
    ...
    switch (c) {
      ...
      case '<':
        {
          // first we check custom special characters for < delimiters and only allow 2 consecutive
          // ones to allow anchor links and HTML processing
  ...
  /**
   * Parse a newline. If it was preceded by two spaces, append a hard line break; otherwise a soft
   * line break.
   *
   * @return true
   */
  ...
  public boolean parseNewline() {
    ...
    // We're gonna add a new node in any case and we need to check the last text node, so flush
    // outstanding text.
    flushTextNode();
    ...
    if (lastChild instanceof Text && (lastChild.getChars().endsWith(" "))) {
      ...
      appendNode(
          spaces >= 2
              ? new HardLineBreak(
                  input.subSequence(
                      index - (options.hardLineBreakLimit ? 3 + crLfDelta : spaces + 1 + crLfDelta),
                      index))
              : new SoftLineBreak(input.subSequence(index - 1 - crLfDelta, index)));
      if (spaces > 0) {
  ...
  /**
   * Attempt to parse backticks, adding either a backtick code span or a literal sequence of
   * backticks.
   *
          ...
          while (lastPos < length) {
            ...
            if (pos < lastPos) {
              SoftLineBreak softLineBreak =
                  new SoftLineBreak(codeText.subSequence(softBreak, lastPos));
  ...
  protected boolean parseDelimiters(DelimiterProcessor delimiterProcessor, char delimiterChar) {
    ...
    this.lastDelimiter =
        new Delimiter(
            input, node, delimiterChar, res.canOpen, res.canClose, this.lastDelimiter, startIndex);
  ...
  /**
   * If next character is [, and ! delimiter to delimiter stack and add a text node to block's
   * children. Otherwise just add a text node.
   *
   * @return true if processed characters false otherwise
   */
    ...
    for (int i = startProc; i < iMax; i++) {
      ...
      if (opener.isImage() && wantBang) {
        ...
        if (textWithBang == null)
          textWithBang =
              input.subSequence(opener.getStartIndex() - 1 - lookAhead, startIndex + lookAhead);
        nodeChars = textWithBang;
      } else {
        if (wantBang
            && opener.getStartIndex() >= lookAhead + 1
            && input.charAt(opener.getStartIndex() - 1 - lookAhead) == '!') {
          if (textWithBang == null)
            textWithBang =
                input.subSequence(opener.getStartIndex() - 1 - lookAhead, startIndex + lookAhead);
          nodeChars = textWithBang;
          ...
          if (textNoBang == null)
            textNoBang =
                input.subSequence(opener.getStartIndex() - lookAhead, startIndex + lookAhead);
  ...
  /**
   * Try to match close bracket against an opening in the delimiter stack. Add either a link or
   * image, or a plain [ character, to block's children. If there is a matching delimiter,
   * removeIndex it from the delimiter stack.
   *
   * <p>Also handles custom link ref processing
   *
   * @return true
   */
  protected boolean parseCloseBracket() {
    ...
    if (peek() == '(') {
      ...
      if ((dest = parseLinkDestination()) != null) {
        if (options.parseMultiLineImageUrls
            && opener.isImage()
            && !dest.startsWith("<")
            && dest.endsWith("?")
            && spnlUrl()) {
    ...
    if (!isLinkOrImage) {
      // maybe reference link, need to see if it matches a custom processor or need to skip this
      // reference because it will be processed on the next char
      ...
      if (linkRefProcessorMatch != null) {
        ...
        // need to figure out max nesting we should test based on what is max processor desire and
        // max available
        ...
        if (maxWanted > nestedBrackets) {
          ...
          while (nested.getPrevious() != null
              && nested.getStartIndex() == nested.getPrevious().getStartIndex() + 1
              && peek(maxAvail) == ']') {
      ...
      if (linkRefProcessorMatch == null) {
        ...
        if (labelLength > 2) {
          ...
          // Empty or missing second label can only be a reference if there's no unescaped bracket
          // in it.
        ...
        if (ref != null) {
          ...
          if (referenceRepository.containsKey(normalizedLabel)) {
            ...
            boolean containsLinks =
                containsLinkRefs(refIsBare ? ref : sequence, opener.getNode().getNext(), false);
            isLinkOrImage = !containsLinks;
            refIsDefined = true;
          } else {
            // need to test if we are cutting in the middle of some other delimiters matching, if we
            // are not then we will make this into a tentative
            if (!opener.isStraddling(ref)) {
              ...
              if (!refIsBare && peek() == '[') {
                ...
                if (nextLength > 0) {
                  // not bare and not defined and followed by another [], roll back to before the
                  // label and make it just text
                  index = beforeLabel;
                } else {
                  // undefined ref, create a tentative one but only if does not contain any other
                  // link refs
                ...
                // undefined ref, bare or followed by empty [], create a tentative link ref but only
                // if does not contain any other link refs
                boolean containsLinks = containsLinkRefs(ref, opener.getNode().getNext(), null);
                if (!containsLinks) {
                  isLinkOrImage = true;
                }
    ...
    if (isLinkOrImage || linkRefProcessorMatch != null) {
      ...
      // Flush text now. We don't need to worry about combining it with adjacent text nodes, as
      // we'll wrap it in a
      // link or image node.
      flushTextNode();
      ...
      if (linkRefProcessorMatch != null) {
        ...
        insertNode =
            ref != null
                ? isImage ? new ImageRef() : new LinkRef()
                : isImage ? new Image() : new Link();
      }
      ...
      if (linkRefProcessorMatch != null) {
        // may need to adjust children's text because some characters were part of the processor's
        // opener/closer
        if (insertNode.hasChildren()) {
          BasedSequence original = insertNode.getChildChars();
          BasedSequence text =
              linkRefProcessorMatch.processor.adjustInlineText(document, insertNode);

          // may need to remove some delimiters if they span across original and changed text
          // boundary or if now they are outside text boundary
          Delimiter delimiter = lastDelimiter;
          while (delimiter != null) {
            Delimiter prevDelimiter = delimiter.getPrevious();

            BasedSequence delimiterChars =
                delimiter
                    .getInput()
                    .subSequence(delimiter.getStartIndex(), delimiter.getEndIndex());
            if (original.containsAllOf(delimiterChars)) {
              if (!text.containsAllOf(delimiterChars)
                  || !linkRefProcessorMatch.processor.allowDelimiters(
                      delimiterChars, document, insertNode)) {
                // remove it
      ...
      if (insertNode instanceof RefNode) {
        ...
        if (!refIsBare) {
          refNode.setTextChars(
              input.subSequence(
                  isImage ? opener.getStartIndex() - 1 : opener.getStartIndex(), startIndex));
        ...
        inlineLinkNode.setTextChars(
            isImage
                ? input.subSequence(opener.getStartIndex() - 1, startIndex)
                : input.subSequence(opener.getStartIndex(), startIndex));
      ...
      // Links within links are not allowed. We found this link, so there can be no other link
      // around it.
      if (insertNode instanceof Link) {
        ...
        if (options.linkTextPriorityOverLinkRef || !containsLinkRefs(insertNode, false)) {
          ...
          collapseLinkRefChildren(
              insertNode, child -> child instanceof LinkRendered || child.isTentative(), true);
          toRemove.unlink();
        } else {
          ...
          appendText(
              insertNode.baseSubSequence(
                  ((Link) insertNode).getTextClosingMarker().getStartOffset(),
                  insertNode.getEndOffset()));
        }
  ...
  protected static boolean containsLinkRefs(Node node, Boolean isTentative) {
    ...
    while (next != null) {
      if (next instanceof LinkRendered
          && (isTentative == null || ((LinkRendered) next).isTentative() == isTentative)) {
        return true;
      }
      next = next.getNext();
    }
  ...
  protected static boolean containsLinkRefs(
    ...
    while (next != null) {
      if (next instanceof LinkRefDerived
          && (isTentative == null || ((LinkRefDerived) next).isTentative() == isTentative)
          && !(next.getStartOffset() >= endOffset || next.getEndOffset() <= startOffset)) {
        return true;
      }
      next = next.getNext();
    }
  ...
  protected static void collapseLinkRefChildren(
    ...
    while (child != null) {
      ...
      if (child instanceof LinkRefDerived
          && (isTentative == null || isTentative.apply((LinkRefDerived) child))) {
    ...
    if (trimFirstLastChild) {
      ...
      if (firstChild == lastChild) {
        if (firstChild != null && !(firstChild instanceof DoNotTrim))
          firstChild.setChars(firstChild.getChars().trim());
      } else {
        if (firstChild != null && !(firstChild instanceof DoNotTrim))
          firstChild.setChars(firstChild.getChars().trimStart());
  ...
  /**
   * Parse a run of ordinary characters, or a single character with a special meaning in markdown,
   * as a plain string.
   *
   * @return true if processed characters false otherwise
   */
  ...
  protected DelimiterData scanDelimiters(
    ...
    String before =
        startIndex == 0 ? SequenceUtils.EOL : String.valueOf(input.charAt(startIndex - 1));
    ...
    canOpen =
        delimiterChar == delimiterProcessor.getOpeningCharacter()
            && delimiterProcessor.canBeOpener(
                before,
                after,
                leftFlanking,
                rightFlanking,
    ...
    canClose =
        delimiterChar == delimiterProcessor.getClosingCharacter()
            && delimiterProcessor.canBeCloser(
                before,
                after,
                leftFlanking,
                rightFlanking,
  ...
  public void processDelimiters(Delimiter stackBottom) {
    ...
    while (closer != null) {
      ...
      if (opener.getNumDelims() == 0) {
        ...
        opener
            .getNode()
            .setChars(opener.getNode().getChars().subSequence(0, opener.getNumDelims()));
      }
  ...
  public void removeDelimiterAndNode(@NotNull Delimiter delim) {
    ...
    if (previousText != null && nextText != null) {
      ...
      previousText.setChars(
          input.baseSubSequence(previousText.getStartOffset(), nextText.getEndOffset()));
      nextText.unlink();
    }
  ...
  public void removeDelimiterKeepNode(@NotNull Delimiter delim) {
    ...
    node =
        delimiterProcessor != null ? delimiterProcessor.unmatchedDelimiterNode(this, delim) : null;
    if (node != null) {
  ...
  static Map<Character, List<InlineParserExtensionFactory>> calculateInlineParserExtensions(
    ...
    for (InlineParserExtensionFactory factory : extensionFactories) {
      ...
      for (int i = 0; i < chars.length(); i++) {
        ...
        List<InlineParserExtensionFactory> list =
            extensionMap.computeIfAbsent(c, k -> new ArrayList<>());
        list.add(factory);
      }
    }
    ...
    for (Character c : extensionMap.keySet()) {
      ...
      List<InlineParserExtensionFactory> resolvedList =
          DependencyResolver.resolveFlatDependencies(list, null, null);
  ...
  public static Map<Character, DelimiterProcessor> calculateDelimiterProcessors(
      DataHolder options, List<DelimiterProcessor> delimiterProcessors) {
    Map<Character, DelimiterProcessor> map = new HashMap<>();
    // addDelimiterProcessors(Arrays.asList(new AsteriskDelimiterProcessor(), new
    // UnderscoreDelimiterProcessor()), map);
    if (Parser.ASTERISK_DELIMITER_PROCESSOR.get(options)) {
      addDelimiterProcessors(
          Collections.singletonList(
              new AsteriskDelimiterProcessor(Parser.STRONG_WRAPS_EMPHASIS.get(options))),
          map);
    }
    if (Parser.UNDERSCORE_DELIMITER_PROCESSOR.get(options)) {
      addDelimiterProcessors(
          Collections.singletonList(
              new UnderscoreDelimiterProcessor(Parser.STRONG_WRAPS_EMPHASIS.get(options))),
          map);
    }
  ...
  public static LinkRefProcessorData calculateLinkRefProcessors(
      DataHolder options, List<LinkRefProcessorFactory> linkRefProcessors) {
    if (linkRefProcessors.size() > 1) {
      List<LinkRefProcessorFactory> sortedLinkProcessors =
          new ArrayList<>(linkRefProcessors.size());
      sortedLinkProcessors.addAll(linkRefProcessors);

      int[] maxNestingLevelRef = new int[] {0};

      Collections.sort(
          sortedLinkProcessors,
          (p1, p2) -> {
  ...
  private static void addDelimiterProcessors(
      List<? extends DelimiterProcessor> delimiterProcessors,
