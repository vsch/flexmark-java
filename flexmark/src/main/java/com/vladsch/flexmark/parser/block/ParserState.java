...
public interface ParserState extends BlockTracker, BlockParserTracker {
  ...
  /**
   * @return the index of the next non-space character starting from {@link #getIndex()} (may be the
   *     same) (0-based)
   */
  int getNextNonSpaceIndex();

  /**
   * The column is the position within the line after tab characters have been processed as 4-space
   * tab stops. If the line doesn't contain any tabs, it's the same as the {@link #getIndex()}. If
   * the line starts with a tab, followed by text, then the column for the first character of the
   * text is 4 (the index is 1).
   *
   ...
   * @return an active block parser for the node or null if not found or the block is already
   *     closed.
   */
  ...
  /**
   * @return the start of line offset into the input stream corresponding to current index into the
   *     line
   */
  int getLineStart();
  ...
  /**
   * @return the end of line offset into the input stream corresponding to current index into the
   *     line, including the EOL
   */
  int getLineEndIndex();

  /**
   * Test the block to see if it ends in a blank line. The blank line can be in the block or its
   * last child.
   *
