...
public interface LinkRefProcessor {
  /**
   * Whether the image ref is desired, if not then ! will be stripped off the prefix and treated as
   * plain text
   *
   ...
   * processor would return 1 Only immediately nested [] are considered. [[ ]] is nesting 1, [ [ ]]
   * is not considered
   *
   * <p>When {@code >0} then preview of next characters is used and if they will match then inner
   * reference will not be created to allow outer one to match the desired element
   *
  ...
  /**
   * Test whether the element matches desired one. For processors that allow nesting this will be
   * called one additional. time for each nesting level. Last call for the actual match, all others
   * to pre-match to prevent a non-nested element from being created.
   *
   ...
   * closing sequence of the node or if the children are not desired then removeIndex them.
   *
   * @param document document, can be used to get parsing options
   * @param node node whose inline text is to be adjusted to reflect some of the text having been
