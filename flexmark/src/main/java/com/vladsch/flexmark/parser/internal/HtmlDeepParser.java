...
public class HtmlDeepParser {
  public enum HtmlMatch {
    ...
    OPEN_TAG(
        "<((?:" + Parsing.XML_NAMESPACE + ")[A-Za-z][A-Za-z0-9-]*" + ")", "<|/>|\\s/>|>", true),
    ...
    HtmlMatch(String open, String close, boolean caseInsentive) {
      this.open =
          open == null ? null : Pattern.compile(open, caseInsentive ? Pattern.CASE_INSENSITIVE : 0);
      this.close =
          close == null
              ? null
              : Pattern.compile(close, caseInsentive ? Pattern.CASE_INSENSITIVE : 0);
  ...
  public boolean isBlankLineInterruptible() {
    return (myOpenTags.isEmpty() && myClosingPattern == null
        || myHtmlMatch == HtmlMatch.OPEN_TAG && myClosingPattern != null && myOpenTags.size() == 1);
  }
