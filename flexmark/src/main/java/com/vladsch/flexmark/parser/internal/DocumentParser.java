...
public class DocumentParser implements ParserState {
  ...
  static {
    ...
    CORE_FACTORIES_DATA_KEYS.put(
        new FencedCodeBlockParser.Factory(), Parser.FENCED_CODE_BLOCK_PARSER);
    ...
    CORE_FACTORIES_DATA_KEYS.put(
        new IndentedCodeBlockParser.Factory(), Parser.INDENTED_CODE_BLOCK_PARSER);
  }
  ...
  static {
    CORE_PARAGRAPH_PRE_PROCESSORS.put(
        Parser.REFERENCE_PARAGRAPH_PRE_PROCESSOR, new ReferencePreProcessorFactory());
  }

  //    final private static HashMap<DataKey<Boolean>, BlockPreProcessorFactory>
  // CORE_BLOCK_PRE_PROCESSORS = new HashMap<>();
  //    static {
  //        //CORE_BLOCK_PRE_PROCESSORS.put(Parser.REFERENCE_PARAGRAPH_PRE_PROCESSOR, new
  ...
  public static List<CustomBlockParserFactory> calculateBlockParserFactories(
      DataHolder options, List<CustomBlockParserFactory> customBlockParserFactories) {
    ...
    for (Map.Entry<CustomBlockParserFactory, DataKey<Boolean>> entry :
        CORE_FACTORIES_DATA_KEYS.entrySet()) {
      ...
      for (DataKey<Boolean> preProcessorDataKey : CORE_PARAGRAPH_PRE_PROCESSORS.keySet()) {
        if (preProcessorDataKey.get(options)) {
          ParagraphPreProcessorFactory preProcessorFactory =
              CORE_PARAGRAPH_PRE_PROCESSORS.get(preProcessorDataKey);
  ...
  private void incorporateLine(BasedSequence ln) {
    ...
    // Check to see if we've hit 2nd blank line; if so break out of list or any other block type
    // that handles this
    if (blank && isLastLineBlank(blockParser.getBlock())) {
      List<BlockParser> matchedBlockParsers =
          new ArrayList<>(activeBlockParsers.subList(0, matches));
        ...
        if (blockParser.getBlock() instanceof KeepTrailingBlankLineContainer) {
          if (blankLine != null) {
            ...
            if (blockParser.isContainer() && lastPrefixClaimer == blockParser) {
              // need to add it as a blank line if it is attributable to the block, otherwise there
              // is no content
              blankLine = new BlankLine(lineWithEOL, blockParser.getBlock());
              blockParser.getBlock().appendChild(blankLine);
            }
  ...
  private void addLine() {
    ...
    // BasedSequence eol = content.baseSubSequence(lineEOL < lineEnd ? lineEnd - 1 : lineEnd,
    // lineEnd).toMapped(EolCharacterMapper.INSTANCE);
  ...
  /**
   * Break out of all containing lists, resetting the tip of the document to the parent of the
   * highest list, and finalizing all the lists. (This is used to implement the "two blank lines
  ...
  /**
   * Add block parser of type T as a child of the currently active parsers. If the tip can't accept
   * children, close and finalize it and try its parent, and so on til we find a block that can
   * accept children.
   *
  ...
  private void preProcessParagraph(
      Paragraph block,
      List<ParagraphPreProcessorFactory> stage,
    ...
    while (true) {
      ...
      for (ParagraphPreProcessorFactory factory : stage) {
        ...
        if (pos > 0) {
          ...
          BasedSequence contentChars =
              blockChars.subSequence(
                  pos
                      + blockChars.countLeading(
                          CharPredicate.WHITESPACE, pos, blockChars.length()));
  ...
  private Document finalizeAndProcess() {
    ...
    //    LOG.debug("Parsed\n" + new
    // AstCollectingVisitor().collectAndGetAstText(documentBlockParser.getBlock()));
    // }
