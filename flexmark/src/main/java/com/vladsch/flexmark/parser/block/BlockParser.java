...
public interface BlockParser {
   ...
   * @param block new block being started @return true if this block parser's block can contain the
   *     given block type, false if it cannot
   */
  ...
  /**
   * @return true if Double blank line should finalize this block parser and its children and reset
   *     to parent
   */
  ...
  /**
   * @return get the currently accumulated block content. May or may not be implemented by any
   *     parser except for the {@link ParagraphParser} or one that returns true for {@link
   *     #isParagraphParser()}
   */
  BlockContent getBlockContent();

  /**
   * Used to clean up and prepare for the next parsing run of the AbstractBlockParser for internal
   * parser house keeping not for BlockParser implementors
   */
  ...
  /**
   * Allows block parsers to keep indenting spaces for those blocks that are interruptible but don't
   * want indenting spaces removed.
   *
