        ...
        if (block instanceof DefinitionItem) {
            ...
            if (previous instanceof Paragraph) {
                ...
                if (definitionTerm != null) {
                    definitionTerm.appendChild(definitionItem);
                ...
                // add to last definition term
                DefinitionList definitionList = (DefinitionList) previous;
                final Node lastChild = definitionList.getLastChild();

                if (lastChild instanceof DefinitionTerm) {
                    final DefinitionTerm definitionTerm = (DefinitionTerm) lastChild;
                    definitionTerm.appendChild(definitionItem);
                    state.blockAddedWithChildren(definitionItem);
                }
            }

            if (definitionItem.getParent() == null) {
                state.blockRemovedWithChildren(definitionItem);
            }
        }
    }
