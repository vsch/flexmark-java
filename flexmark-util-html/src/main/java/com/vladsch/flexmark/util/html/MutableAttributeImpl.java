...
public class MutableAttributeImpl implements MutableAttribute {
        ...
        if (!value.isEmpty()) {
          ...
          while (lastPos < value.length()) {
            ...
            if (lastPos < endPos) {
              ...
              int namePos =
                  valueNameDelimiter != SequenceUtils.NUL ? value.indexOf(valueNameDelimiter) : -1;
  ...
  /**
   * Return the attribute value string by splicing the values of the map using valueListDelimiter
   * and valueNameDelimiter with replacements of the given name/value if provided. If the name is
   * not empty and value is empty then this will be removed from the final string
   *
    ...
    while (lastPos < useValue.length()) {
      ...
      if (lastPos < endPos) {
        ...
        if (!valueItem.isEmpty()) {
          int namePos =
              valueNameDelimiter == SequenceUtils.NUL ? -1 : valueItem.indexOf(valueNameDelimiter);
  ...
  public static MutableAttributeImpl of(
      CharSequence attrName, CharSequence value, char valueListDelimiter) {
