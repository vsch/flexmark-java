...
public interface BasedOptionsHolder {
    ...
    int F_COLLECT_FIRST256_STATS = BitEnumSet.intMask(O_COLLECT_FIRST256_STATS);      // collect statistics for segments sequence on chars < code 256
    ...
    int F_FULL_SEGMENTED_SEQUENCES = BitEnumSet.intMask(O_FULL_SEGMENTED_SEQUENCES);  // do not include anchors in segment builder, test only, not guaranteed to be stable for general use
    int F_TREE_SEGMENTED_SEQUENCES = BitEnumSet.intMask(O_TREE_SEGMENTED_SEQUENCES);  // do not include anchors in segment builder, test only, not guaranteed to be stable for general use
     ...
     * @return true if all option flags are set for this sequence
     */
     ...
     *
     * @param options option flags
     * @return true if any option flag is set for this sequence
     */
