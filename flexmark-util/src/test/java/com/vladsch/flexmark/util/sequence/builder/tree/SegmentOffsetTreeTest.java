...
public class SegmentOffsetTreeTest {
  ...
  public void test_build1SegmentTree() {
    ...
    assertEquals(
        "BasedSegmentBuilder{[0, 10), s=0:0, u=0:0, t=0:0, l=6, sz=4, na=2: [0), [2, 5), [6, 9), [10) }",
        segments.getSegmentBuilder().toStringPrep());
  ...
  public void test_optimizersCompound3Anchors() {
    ...
    SegmentTree segTree = SegmentTree.build(segments.getSegments(), segments.getText());
    assertEquals(
        "SegmentTree{aggr: {[13, 0:], [22, 3:] }, seg: { 0:[0, 13), 3:[14, 23) } }",
        segTree.toString(sequence));
    ...
    BasedSegmentBuilder segments2 =
        BasedSegmentBuilder.emptyBuilder(sequence, F_TRACK_FIRST256 | F_INCLUDE_ANCHORS);
    SegmentTreeRange treeRange =
        segTree.getSegmentRange(5, segments.length() - 5, 0, segTree.size(), sequence, null);
    segTree.addSegments(segments2, treeRange);
